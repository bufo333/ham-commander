0 rem ================================
1 rem   ham commander v2.0
2 rem   c64 ham radio logger
3 rem   with local storage + online
4 rem ================================
5 rem
6 rem f1=spots f3=log f5=new f7=online
7 rem
8 rem ── initialize ────────────────
...
17 mn$="":mg$="":rem operator name/grid
18 ol=0:rem online flag (0=off 1=on)
19 sc=0:rem screen (0=log 1=spots 2=new 3=detail 4=filter)
20 rc=0:rem total record count
21 li$="0":rem last qrz logid
22 pq=0:rem pending queue count
23 sp=0:rem spot count in ram
24 lp=0:rem log page (0-based)
25 sl=0:rem selected line on screen
26 fi=0:rem filter index count
27 fb$="":fm$="":rem filter band/mode
28 ut$="00000000":uh$="0000":rem utc date/time
29 s9$="                                                                                   ":rem 83 spaces
30 rem ── dim spot arrays (80 max) ─
31 dim ca$(40),fq$(40),mo$(40),rf$(40),sd$(40)
32 rem ca$=call fq$=freq mo$=mode rf$=park sd$=detail
33 rem
34 rem ── dim log page cache (20) ──
35 dim xc$(20),xb$(20),xd$(20),xr$(20),xl$(20)
36 rem xc$=call xb$=band|mode|freq xd$=date|time
37 rem xr$=rst xl$=logid|grid|name|country|comment|sync
38 rem
39 rem ── dim filter index (80) ────
40 dim fx(40):rem index into spot arrays
41 rem
49 rem ── load config file ─────────
50 gosub 2400
51 rem ── load index file ──────────
52 gosub 2450
53 rem ── count pending queue ──────
54 gosub 2480
55 print " station: ";mc$
...
258 rem
259 rem ── spots scroll down ─────────
260 mx=fi-1:if mx<0 then mx=0
261 if sl<mx then sl=sl+1:gosub 300
262 return
263 rem
269 rem ── spots scroll up ───────────
270 if sl>0 then sl=sl-1:gosub 300
271 return
272 rem
279 rem ── spots select (enter) ──────
280 if fi=0 then return
281 ix=fx(sl+1):rem get actual spot index
282 rem ── pre-fill new qso ──────────
283 nc$=ca$(ix):nf$=fq$(ix):nm$=mo$(ix)
284 nb$="":rem calc band from freq
285 gosub 1280:rem freq to band
286 sc=2:gosub 1000
287 return
288 rem
289 rem ── build filter index ─────────
290 fi=0
291 for i=1 to sp
292 fi=fi+1:fx(fi)=i
293 next i
294 return
295 rem
299 rem ── draw spot list ────────────
300 gosub 2200:rem title bar
301 rem ── status line ───────────────
302 st$=""
...
308 rem ── separator ──────────────────
309 print chr$(154);"---------------------------------------";chr$(5)
310 rem ── calculate visible window ───
311 vw=19:rem visible rows
312 tp=sl:if tp>fi-vw then tp=fi-vw
313 if tp<0 then tp=0
314 rem ── draw rows ──────────────────
315 for i=0 to vw-1
316 ri=tp+i+1:rem 1-based index into fx
317 if ri>fi then print "                                        ";:goto 330
318 ix=fx(ri):rem actual spot array index
319 rem ── highlight selected ─────────
320 if tp+i=sl then print chr$(158);chr$(18);:rem yellow reverse
321 cl$=left$(ca$(ix)+"          ",10)
322 fr$=left$(fq$(ix)+"       ",7)
323 md$=left$(mo$(ix)+"    ",4)
324 pr$=left$(rf$(ix)+"          ",10)
325 print cl$;fr$;" ";md$;" ";pr$;
326 if tp+i=sl then print chr$(146);chr$(5);:rem reverse off + white
327 print
328 rem
330 next i
331 rem ── separator + help ───────────
332 print chr$(154);"---------------------------------------";chr$(5)
333 print chr$(155);" enter=log  f2=filter  ";chr$(17);chr$(145);"=scroll";chr$(5)
334 gosub 2260:rem bottom bar
335 return
336 rem
499 rem ================================
500 rem   log screen
501 rem ================================
502 gosub 2200:rem title bar
503 rem ── status line ────────────────
504 st$=""
...
510 print chr$(154);"---------------------------------------";chr$(5)
511 rem ── load page from rel file ────
512 if rc=0 then print:print "  no qsos yet. press f5 to add.":gosub 2260:sl=0:return
513 gosub 1900:rem load page lp into cache
514 rem ── draw log entries ───────────
515 sl=0:vw=19
516 gosub 540:rem draw log rows
517 return
518 rem
539 rem ── draw log rows ──────────────
540 gosub 2200:gosub 2220
542 pg=lp*19:rem first record of this page
543 rem ── how many on this page ──────
544 pc=19:if pg+pc>rc then pc=rc-pg
545 if pc<0 then pc=0
546 for i=0 to vw-1
547 if i>=pc then print:goto 555
548 if i=sl then print chr$(158);chr$(18);:rem yellow reverse
549 rem ── format: date time call band mode
550 gosub 590:rem format log line i
551 print ln$;
552 if i=sl then print chr$(146);chr$(5);
553 print
555 next i
556 print chr$(154);"---------------------------------------";chr$(5)
557 print chr$(155);" pg ";lp+1;" enter=detail ";chr$(17);chr$(145);"=scroll";chr$(5)
558 gosub 2260
559 return
560 rem ── log scroll down ────────────
...
566 return
567 rem
570 rem ── log scroll up ──────────────
571 if sl>0 then os=sl:sl=sl-1:gosub 600:return
572 rem ── prev page? ─────────────────
573 if lp>0 then lp=lp-1:gosub 1900:sl=18:gosub 540
574 return
575 rem
579 rem ── log select (enter=detail) ──
580 if rc=0 then return
581 di=lp*19+sl:rem record index (0-based)
582 if di>=rc then return
583 sc=3:gosub 800
584 return
585 rem
589 rem ── format log line for row i ──
590 rem xd$="yyyymmdd|hhmm" xc$=call xb$="band|mode|freq" xr$="rsts|rstr"
591 d$=left$(xd$(i+1),8)
592 if len(xd$(i+1))>9 then t$=mid$(xd$(i+1),10,4):goto 594
593 t$="----"
594 cl$=left$(xc$(i+1)+"         ",9)
595 bd$=left$(xb$(i+1),3)
596 md$=left$(mid$(xb$(i+1),6,3)+"   ",3)
598 ln$=mid$(d$,5,2)+"/"+mid$(d$,7,2)+" "+t$+" "+cl$+bd$+" "+md$
599 return
600 rem ── fast scroll: update 2 rows ──
601 rem os=old selected, sl=new selected
602 rem row offset: poke 214,n:print positions to row n+1
603 i=os:gosub 590:poke 214,os+1:print
...
608 rem
799 rem ================================
800 rem   log detail screen
801 rem ================================
802 gosub 2200:rem title bar
803 print "  loading detail..."
804 rem ── read full record from hamlog.dat ─
805 rn=lp*19+sl+1:rem 1-based record number
806 open 15,8,15
807 open 3,8,3,"hamlog.dat,l,"+chr$(168)
...
813 rem ── display detail from w$ ────
814 gosub 2200:print "  qso detail"
817 print chr$(154);"---------------------------------------";chr$(5)
818 print
...
837 print chr$(154);"---------------------------------------";chr$(5)
838 print chr$(155);" del=back  d=delete qso";chr$(5)
839 gosub 2260
840 return
841 rem
842 rem ── delete qso handler ──────────
843 print chr$(147);chr$(5);
844 print "  delete this qso?"
845 print "  ";xc$(ci);" ";left$(xd$(ci),8)
846 print
847 print "  y=delete  n=cancel"
848 get w$:if w$="" then 848
849 if w$<>"y" then sc=3:gosub 800:return
850 rem ── mark record as deleted ───────
851 di=lp*19+sl:rn=di+1:gosub 890:return
852 rem ── parse pipe-delimited xb$ ───
853 b1$="":b2$="":b3$="":bn=1
854 for bj=1 to len(tb$)
855 bc$=mid$(tb$,bj,1)
856 if bc$="|" then bn=bn+1:goto 859
857 if bn=1 then b1$=b1$+bc$
858 if bn=2 then b2$=b2$+bc$
859 if bn=3 then b3$=b3$+bc$
860 next bj:return
861 rem
862 rem ── parse pipe-delimited xr$ ───
863 r1$="":r2$="":rn=1
864 for rj=1 to len(tr$)
865 rk$=mid$(tr$,rj,1)
866 if rk$="|" then rn=rn+1:goto 868
867 if rn=1 then r1$=r1$+rk$
868 if rn=2 then r2$=r2$+rk$
869 next rj:return
870 rem
871 rem ── parse pipe-delimited xl$ ───
872 l1$="":l2$="":l3$="":l4$="":l5$="":l6$=""
873 ln=1
874 for lj=1 to len(tl$)
875 lc$=mid$(tl$,lj,1)
876 if lc$="|" then ln=ln+1:goto 880
877 if ln=1 then l1$=l1$+lc$
878 if ln=2 then l2$=l2$+lc$
879 if ln=3 then l3$=l3$+lc$
880 if ln=4 then l4$=l4$+lc$
881 if ln=5 then l5$=l5$+lc$
882 if ln=6 then l6$=l6$+lc$
883 next lj:return
884 rem
889 rem ── delete qso from both files ──
890 open 15,8,15
891 open 3,8,3,"hamlog.dat,l,"+chr$(168)
...
899 close 3:open 3,8,3,"hamlog.sum,l,"+chr$(40)
...
906 rem
999 rem ================================
1000 rem   new qso screen
1001 rem ================================
1002 gosub 2200:rem title bar
1003 print "  new qso entry"
1004 print chr$(154);"---------------------------------------";chr$(5)
1005 rem ── pre-filled values ──────────
1006 rem nc$=call nf$=freq nm$=mode nb$=band (from spots)
1007 if nc$="" then print:input "  callsign: ";nc$
1008 if nc$<>"" then print "  callsign: ";nc$
1009 if nc$="" then sc=0:gosub 500:return
1010 rem
1011 rem ── lookup callsign if online ─
1012 lk$="":lg$="":ln$="":ly$=""
...
1039 rem ── name/grid from lookup ──────
1040 na$=ln$:gr$=lg$:cy$=ly$
1041 if gr$="" then gr$=mg$
1042 rem ── confirm ────────────────────
1043 print:print chr$(154);"---------------------------------------";chr$(5)
1044 print "  ";nc$;" ";nb$;" ";nm$
1045 print "  ";nd$;" ";nt$;" utc"
1046 print "  rst: ";rs$;" / ";rr$
1047 if na$<>"" then print "  name: ";na$
1048 if co$<>"" then print "  note: ";left$(co$,30)
1049 print chr$(154);"---------------------------------------";chr$(5)
1050 print "  y=save  n=cancel"
1051 get w$:if w$="" then 1051
1052 if w$<>"y" and w$<>"y" then nc$="":nf$="":nm$="":nb$="":sc=0:gosub 500:return
1053 rem
1054 rem ── save to rel file ───────────
1055 rc=rc+1:rem increment record count
1056 gosub 1100:rem build + write record
1057 gosub 1130:rem write summary record
1058 gosub 1150:rem write to hamlog.que
1059 pq=pq+1
1060 rem ── update index file ──────────
1061 gosub 2460
1062 rem
1063 print:print "  qso saved! #";rc
1064 if pq>0 and ol=0 then print "  pending sync: ";pq
1065 rem ── if online, upload now ──────
1066 if ol=1 then gosub 1170
1067 for w=1 to 1000:next w
1068 rem ── clear pre-fill ─────────────
1069 nc$="":nf$="":nm$="":nb$=""
1070 sc=0:gosub 500
1071 return
1072 rem
1099 rem ── build record + write rel ──
1100 rem pack 167-byte record (print# adds cr at 168)
1101 w$=left$(nc$+"            ",12)
1102 w$=w$+left$(nb$+"      ",6)
1103 w$=w$+left$(nm$+"      ",6)
1104 w$=w$+left$(nd$+"        ",8)
...
1108 w$=w$+left$(rr$+"   ",3)
1109 w$=w$+left$(mc$+"            ",12)
1110 w$=w$+left$("            ",12):rem qrz_logid (empty)
1111 w$=w$+"n":rem sync_flag = new
1112 w$=w$+left$(co$+"                                        ",40)
1113 w$=w$+left$(gr$+"      ",6)
1114 w$=w$+left$(na$+"                              ",30)
1115 w$=w$+left$(cy$+"             ",13)
1116 rem ── write to rel file ──────────
1117 open 15,8,15
1118 open 3,8,3,"hamlog.dat,l,"+chr$(168)
1119 rem ── position to record rc ──────
...
1138 return
1139 rem
1149 rem ── append to sync queue ───────
1150 open 4,8,4,"hamlog.que,s,a"
1151 rem csv: call,band,mode,freq,date,time,rsts,rstr,comment,recnum
1152 print#4,nc$;",";nb$;",";nm$;",";nf$;",";nd$;",";nt$;",";rs$;",";rr$;",";co$;",";rc
1153 close 4
1154 return
1155 rem
1169 rem ── upload single qso (online) ─
1170 cm$="add,"+nc$+","+nb$+","+nm$+","+nf$+","+nd$+","+nt$+","+rs$+","+rr$+","+co$
1171 gosub 2000:rem send command
1172 gosub 2010:rem read response
1173 if left$(rl$,7)="!add,ok" then gosub 1180:return
1174 print "  upload failed - queued"
1175 return
1176 rem
1179 rem ── update record with logid ───
1180 rem extract logid from !add,ok,12345
1181 lg$=mid$(rl$,9)
1182 rem ── update rel record sync flag ─
1183 open 15,8,15
1184 open 3,8,3,"hamlog.dat,l,"+chr$(168)
...
1192 close 3:open 3,8,3,"hamlog.sum,l,"+chr$(40):print#15,"p"+chr$(3)+chr$(lo)+chr$(hi)+chr$(1)
...
1196 return
1197 rem
1199 rem ── online callsign lookup ─────
1200 print "  looking up ";nc$;"..."
1201 cm$="lookup,"+nc$
1202 gosub 2000:rem send
1203 gosub 2010:rem read status line
1204 if left$(rl$,10)<>"!lookup,ok" then print "  not found":return
1205 gosub 2010:rem read data line
1206 rem ── parse: call,name,addr,city,state,country,grid,...
1207 gosub 240:rem reuse csv parser (p1$-p7$)
1208 ln$=p2$:rem name
1209 lg$=p7$:rem grid (field 7)
1210 rem ── need field 6 = country ─────
1211 ly$=p6$
1212 if ln$<>"" then print "  name: ";ln$
1213 if lg$<>"" then print "  grid: ";lg$
1214 gosub 2010:rem read !end
1215 return
1216 rem
1279 rem ── freq to band lookup ────────
1280 fv=val(nf$)
1281 nb$=""
...
1294 rem
1299 rem ================================
1300 rem   filter dialog
1301 rem ================================
1302 print chr$(147);chr$(5);
1303 print chr$(154);"=======================================";chr$(5);
1304 print "         spot filter"
1305 print chr$(154);"=======================================";chr$(5);
1306 print
1307 print "  band:"
...
1343 rem ── re-fetch spots with filter ─
1344 sc=1:gosub 200
1345 return
1346 rem
1499 rem ================================
...
1504 print chr$(154);"---------------------------------------";chr$(5)
1505 rem ── open modem ─────────────────
1506 print " opening modem..."
1507 open 2,2,0,chr$(8)+chr$(0):rem 1200 baud
1508 rem ── dial server ────────────────
1509 print " dialing ";sv$;":";pt$
1510 print#2,"atdt"+sv$+":"+pt$+chr$(13);
1511 rem ── wait for connection ─────────
...
1522 gosub 2010:rem read response
1523 print " resp: ";left$(rl$,30)
1524 if left$(rl$,3)<>"!ok" then print " connection failed":goto 1540
1525 rem ── parse !ok,c64hamlog,2.0,date,time ─
1526 gosub 240:rem parse csv (p1$-p7$)
1527 rem p1$=!ok p2$=c64hamlog p3$=ver p4$=date p5$=time
1528 rem
1529 ut$=p4$:uh$=p5$
1530 ol=1
...
1562 rem ── update rel record ───────────
1563 rn=val(qn$):if rn>0 and left$(rl$,7)="!add,ok" then gosub 1590
1564 goto 1554
1565 rem
1579 rem ── done uploading ─────────────
1580 close 4
1581 pq=pq-uq:if pq<0 then pq=0
1582 print " uploaded: ";uq;" qsos"
1583 rem ── clear queue file ────────────
1584 if pq=0 then open 15,8,15,"s0:hamlog.que":close 15
1585 gosub 2460:rem update index
1586 return
1587 rem
1589 rem ── update rel record sync ─────
1590 lg$=mid$(rl$,9):rem extract logid
1591 open 15,8,15
1592 open 3,8,3,"hamlog.dat,l,"+chr$(168)
...
1600 close 3:open 3,8,3,"hamlog.sum,l,"+chr$(40):print#15,"p"+chr$(3)+chr$(lo)+chr$(hi)+chr$(1)
...
1704 print chr$(154);"---------------------------------------";chr$(5)
1705 rem ── send bye ───────────────────
1706 cm$="bye":gosub 2000
1707 gosub 2010:rem read !bye response
1708 rem ── hayes escape + hangup ──────
...
1722 rem
1749 rem ================================
1750 rem   sync logbook (f4 online)
1751 rem ================================
1752 gosub 2200
1753 print "  syncing logbook..."
1754 print chr$(154);"---------------------------------------";chr$(5)
1755 rem ── upload pending queue first ────
1756 if pq>0 then gosub 1550
1757 rem ── send fetch command ──────────
1758 cm$="fetch":gosub 2000
1759 gosub 2010:rem read !log,count
1760 if left$(rl$,4)<>"!log" then print " sync failed: ";rl$:goto 1795
1761 sn=val(mid$(rl$,6)):rem count
1762 print " received ";sn;" qsos from qrz"
1763 if sn=0 then gosub 2010:goto 1795
1764 rem ── read and store each record ──
1765 open 15,8,15:open 3,8,3,"hamlog.dat,l,"+chr$(168):open 5,8,5,"hamlog.sum,l,"+chr$(40)
1766 sa=0:for si=1 to sn
1767 gosub 2010:gosub 240
1768 rem p1$=logid p2$=call p3$=band p4$=mode p5$=freq p6$=date p7$=time
1769 rem need more fields - read raw rl$
1770 rem ── build record from csv ────────
...
1794 gosub 2010:rem read !end
1795 gosub 2460:rem save index
1796 print
...
1802 rem
1899 rem ================================
1900 rem   load page from summary file
1901 rem ================================
1902 rem load 18 summaries from hamlog.sum (40-byte records)
1903 if rc=0 then return
1904 open 15,8,15
1905 open 3,8,3,"hamlog.sum,l,"+chr$(40)
1906 ps=lp*19:rem start offset (0-based)
1907 pc=19:if ps+pc>rc then pc=rc-ps
1908 if pc<1 then close 3:close 15:return
1909 for i=1 to pc
1910 rn=ps+i:rem record number (1-based)
1911 lo=rn and 255:hi=int(rn/256)+1
1912 print#15,"p"+chr$(3)+chr$(lo)+chr$(hi)+chr$(1)
1913 input#3,w$
1914 rem ── unpack summary: call10+band4+mode4+date8+time4+rsts3+rstr3+flag1+pad2
1915 xc$(i)=left$(w$,10)
1916 xb$(i)=mid$(w$,11,4)+"|"+mid$(w$,15,4)+"|"
1917 xd$(i)=mid$(w$,19,8)+"|"+mid$(w$,27,4)
1918 xr$(i)=mid$(w$,31,3)+"|"+mid$(w$,34,3)
1919 xl$(i)="":rem detail loaded on demand
1920 next i
1921 close 3:close 15
1922 return
1923 rem
1999 rem ================================
2000 rem   protocol: send command
2001 rem ================================
2002 print#2,cm$+chr$(13);
2003 return
2004 rem
2009 rem ── protocol: read response line ─
2010 rl$="":rt=0
2011 get#2,a$
2012 if a$="" or a$=chr$(0) then rt=rt+1:if rt>5000 then return
2013 if a$="" or a$=chr$(0) then 2011
2014 rt=0:if a$=chr$(10) then 2011:rem skip lf
2015 if a$=chr$(13) then return:rem end of line
2016 if len(rl$)<250 then rl$=rl$+a$
2018 goto 2011
2199 rem ================================
2200 rem   screen utils - title bar
2201 rem ================================
2202 print chr$(147);chr$(5);:rem clear + white
2203 print chr$(154);chr$(18);"                                       ";chr$(146)
2204 rem ── position for title ────────
2205 print chr$(19);:rem home
...
2226 return
2227 rem
2259 rem ── bottom f-key bar ───────────
2260 rem position to row 24 (last row)
2261 poke 214,23:print:rem move to row 24
2262 print chr$(154);chr$(18);"f1=spt f3=log f4=cfg f5=nw f6=syn f7=on";chr$(146);chr$(5);
2263 return
2264 rem
2399 rem ================================
2400 rem   config file load
2401 rem ================================
2402 open 15,8,15
...
2492 rem
2499 rem ================================
2500 rem   first-run setup wizard
2501 rem ================================
2502 close 4:close 15
2503 print chr$(147);chr$(5);
2504 print chr$(154);"=======================================";chr$(5)
2505 print "       station setup"
2506 print chr$(154);"=======================================";chr$(5)
2507 print
2508 print " no logbook found. let's set up"
2509 print " your station."
2510 print
2511 rem ── callsign (required) ─────────
2512 input " your callsign: ";mc$
2513 if mc$="" then print " callsign required!":goto 2512
2514 print
2515 rem ── try online lookup? ──────────
2516 print " look up on qrz? (y/n)"
2517 get w$:if w$="" then 2517
2518 if w$<>"y" then 2550
2519 rem ── quick online for lookup ─────
2520 print:print " server ip [";sv$;"]:"
2521 input " ";ts$:if ts$<>"" then sv$=ts$
2522 print " port [";pt$;"]:"
2523 input " ";ts$:if ts$<>"" then pt$=ts$
2524 print:print " connecting..."
2525 open 2,2,0,chr$(8)+chr$(0)
2526 print#2,"atdt"+sv$+":"+pt$+chr$(13);
2527 cf=0:for w=1 to 2000
2528 get#2,a$:if a$>=chr$(32) and a$<chr$(127) then cf=1
2529 if a$=chr$(13) and cf=1 then w=2000
2530 next w
2531 cm$="hello":gosub 2000
2532 gosub 2010
2533 if left$(rl$,3)<>"!ok" then print " connect failed":close 2:goto 2550
2534 gosub 240:ut$=p4$:uh$=p5$
2535 rem ── lookup callsign ─────────────
2536 cm$="lookup,"+mc$:gosub 2000
2537 gosub 2010
2538 if left$(rl$,10)<>"!lookup,ok" then print " not found on qrz":close 2:goto 2550
...
2584 rem
2599 rem ================================
2600 rem   config editor (f6)
2601 rem ================================
2602 print chr$(147);chr$(5);
2603 print chr$(154);"=======================================";chr$(5)
2604 print "       edit configuration"
2605 print chr$(154);"=======================================";chr$(5)
2606 print
...
2618 if w$="1" then input " callsign: ";mc$:goto 2602
2619 if w$="2" then input " name: ";mn$:goto 2602
2620 if w$="3" then input " grid: ";mg$:goto 2602
2621 if w$="4" then input " server ip: ";sv$:goto 2602
2622 if w$="5" then input " port: ";pt$:goto 2602
2623 if w$="6" then input " baud: ";bd$:goto 2602
2624 goto 2616
2625 rem
2649 rem ── save and return ─────────────
2650 print:print " saving..."
2651 gosub 2430:rem save config
...
2656 rem
2699 rem ================================
2700 rem   splash screen
2701 rem ================================
2702 poke 53280,6:poke 53281,0
...
2727 rem ── color cycle title row ─────
